...
## Read Data
ds = xr.open_dataset("../SourceData/Fig1a_population_density.nc")
population_density = ds["population_density"].values
lat = ds["lat"].values
lon = ds["lon"].values
extent2 = [lon.min(), lon.max(), lat.min(), lat.max()]
...
SHP = r'../SourceData/shpfile_China'
def make_map(ax,box):
...
mycmap3 = ListedColormap([
    [1, 1, 1, 1.0],
    [0.681399, 0.681399, 0.681399, 1.0],
    [0.10882399, 0.10882399, 0.10882399, 1.0]
], name='mycmap3')
...
## Population
pop_cmap=mycmap3
pop_bounds = [0, 300, 1500, 20000]
pop_norm = mcolors.BoundaryNorm(pop_bounds, pop_cmap.N)
pop_labels = ["Rural cells", "Urban Cluster", "Urban Centre"]
im1 = ax1.imshow(population_density, extent=extent2, cmap=pop_cmap, norm=pop_norm,
